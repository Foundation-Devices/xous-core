...
impl PddbOs {
    ...
    fn syskey_ensure(&mut self) {
        while self.try_login() != PasswordState::Correct {
            ...
            // TODO This should return an error instead and probably shouldn't be a loop
        }
    }
    ...
    // TODO This is a password confirmation procedure. Clearly does not belong here.
    // The UX should take care of it. Once again, this should not be in a loop.
    // So despite the misleading method name, this does not check the password.
    // It just confirms that the user enters the same password twice.
    //
    // It seems to attempt to avoid holding the password in RAM but that already has
    // to happen at some point right? What's the point of this?
    /*
